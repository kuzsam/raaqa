...
REQUIRED_WIN_COLS = {"Chromosome", "Start", "End", "Total_Bases", "Median_MAPQ", "Mean_MAPQ", "Softclip_%"}
REQUIRED_SUM_COLS = {"Chromosome", "Mean_MAPQ", "Median_MAPQ", "Softclip_%"}

def _check_columns(df, required, filename):
    missing = required - set(df.columns)
    if missing:
        sys.exit(f"[ERROR] {filename} is missing required columns: {', '.join(sorted(missing))}")
...
def parse_arguments():
    ...
    parser.add_argument(
        "-m", "--module", required=True, choices=["mapq_softclip", "hese"],
...
def _apply_genome_xticks(ax, chrom_ticks, chrom_labels):
    ...
    ax.set_xticklabels(chrom_labels, rotation=90, fontsize=4.5,
                       color=TEXT_DIM, fontfamily=FONT_MAIN)
...
def fig_genome_three_panel(df_win, df_sum, figures_folder):
    ...
    if genome_row.empty:
        print("[WARN] No GENOME row found in summary_stats.csv — genome-wide reference lines will be omitted.")
    genome_mean_mapq  = float(genome_row["Mean_MAPQ"].iloc[0])   if not genome_row.empty else np.nan
    genome_med_mapq   = float(genome_row["Median_MAPQ"].iloc[0]) if not genome_row.empty else np.nan
    genome_softclip   = float(genome_row["Softclip_%"].iloc[0])  if not genome_row.empty else np.nan
    ...
    genome_pos, dividers, chrom_ticks, chrom_labels, chrom_x = _build_genome_xaxis(df, chroms)
    ...
    for chrom in chroms:
        ...
        contig_means.append(float(row["Mean_MAPQ"].iloc[0])      if not row.empty else np.nan)
        contig_meds.append(float(row["Median_MAPQ"].iloc[0])     if not row.empty else np.nan)
        contig_softclips.append(float(row["Softclip_%"].iloc[0]) if not row.empty else np.nan)
    ...
    ax_cov.axhline(mean_cov,   **MEAN_STYLE_DASH,   label=f"Genome mean coverage  {mean_cov:.1f}×")
    ax_cov.axhline(median_cov, **MEDIAN_STYLE_DASH, label=f"Genome median coverage  {median_cov:.1f}×")

    ax_cov.plot(contig_mids, contig_cov_means, color=COV_A, linewidth=1.2,
                linestyle="-", zorder=4, label="Mean coverage per contig")
    ax_cov.plot(contig_mids, contig_cov_meds,  color=COV_B, linewidth=1.2,
                linestyle="-", zorder=4, label="Median coverage per contig")
    ...
    ax_mapq.plot(contig_mids, contig_means, color=MAPQ_B, linewidth=1.2,
                 linestyle="-", zorder=4, label="Mean MAPQ per contig")
    ax_mapq.plot(contig_mids, contig_meds,  color=MAPQ_A, linewidth=1.2,
                 linestyle="-", zorder=4, label="Median MAPQ per contig")
    ...
    ax_mapq.axhline(genome_mean_mapq, **MEAN_STYLE_DASH,
                    label=f"Genome mean MAPQ  {genome_mean_mapq:.2f}")
    ax_mapq.axhline(genome_med_mapq,  **MEDIAN_STYLE_DASH,
                    label=f"Genome median MAPQ  {genome_med_mapq:.1f}")

    ax_sc.plot(contig_mids, contig_softclips, color=SC_A, linewidth=1.2,
               linestyle="-", zorder=4, label="Softclip % per contig")
    ax_sc.scatter(contig_mids, contig_softclips, color=SC_A, s=12, zorder=5)
    ax_sc.axhline(genome_softclip, **{**SOFTCLIP_STYLE, "linestyle": "--"},
                  label=f"Genome softclip %  {genome_softclip:.2f}")

    cov_vals = [v for v in contig_cov_means + contig_cov_meds + [mean_cov, median_cov]
                if np.isfinite(v)]
    ...
    ax_cov.set_title("Coverage, MAPQ & Softclip Summary — Genome-wide",
                     color=TEXT, fontsize=13, fontweight="bold", fontfamily=FONT_MAIN)
    ax_cov.legend(fontsize=9, facecolor=AX_BG, edgecolor=GRID, labelcolor=TEXT, framealpha=0.9,
                  loc="upper left", bbox_to_anchor=(1.01, 1), bbox_transform=ax_cov.transAxes)

    mapq_data_max = np.nanmax([*contig_means, *contig_meds, genome_mean_mapq, genome_med_mapq])
    ylim_top_mapq = max(65, mapq_data_max * 1.1) if np.isfinite(mapq_data_max) else 65
    ax_mapq.set_ylim(0, ylim_top_mapq)
    ax_mapq.set_ylabel("MAPQ per contig", color=TEXT_DIM, fontsize=10, fontfamily=FONT_MAIN)
    ax_mapq.legend(fontsize=9, facecolor=AX_BG, edgecolor=GRID, labelcolor=TEXT, framealpha=0.9,
                   loc="upper left", bbox_to_anchor=(1.01, 1), bbox_transform=ax_mapq.transAxes)

    ax_sc.set_ylim(bottom=0)
    ax_sc.set_ylabel("Softclip % per contig", color=TEXT_DIM, fontsize=10, fontfamily=FONT_MAIN)
    _apply_genome_xticks(ax_sc, chrom_ticks, chrom_labels)
    ax_sc.legend(fontsize=9, facecolor=AX_BG, edgecolor=GRID, labelcolor=TEXT, framealpha=0.9,
                 loc="upper left", bbox_to_anchor=(1.01, 1), bbox_transform=ax_sc.transAxes)
...
def fig_per_contig_raw(df_win, df_sum, figures_folder):
    ...
    if df.empty:
        print("[WARN] No per-contig data found in window_stats.csv — skipping raw contig figures.")
        return
    ...
    for i, chrom in enumerate(chroms, 1):
        ...
        ax_cov.axhline(mean_cov,   **MEAN_STYLE,   label=f"Contig mean cov  {mean_cov:.1f}×")
        ax_cov.axhline(median_cov, **MEDIAN_STYLE, label=f"Contig median cov  {median_cov:.1f}×")

        overflow_cov = cov > ylim_top_cov
        if overflow_cov.any():
            cov_clip = cov[overflow_cov]
            ax_cov.scatter(x[overflow_cov], np.full(overflow_cov.sum(), ylim_top_cov * 0.96),
                           marker="^", color=COV_LINE, s=20, zorder=6, alpha=0.35,
        ...
        ax_cov.legend(fontsize=8, facecolor=AX_BG, edgecolor=GRID,
        ...
        mapq_data_max = np.nanmax([*mean_mapq_win, *med_mapq_win, contig_mean_mapq, contig_med_mapq])
        ylim_top_mapq = max(65, mapq_data_max * 1.1) if np.isfinite(mapq_data_max) else 65

        # — Median MAPQ panel — pre-computed per window —
        ax_med.fill_between(x, med_mapq_win, color=MAPQ_A, alpha=0.95, linewidth=0)
        ax_med.axhline(contig_med_mapq, **MEDIAN_STYLE,
                       label=f"Contig median MAPQ  {contig_med_mapq:.1f}")
        ax_med.set_ylabel("Median MAPQ", color=TEXT_DIM, fontsize=9, fontfamily=FONT_MAIN)
        ax_med.set_ylim(0, ylim_top_mapq)
        ax_med.legend(fontsize=8, facecolor=AX_BG, edgecolor=GRID,
                      labelcolor=TEXT, framealpha=0.9,
                      loc="upper left", bbox_to_anchor=(1.01, 1), bbox_transform=ax_med.transAxes)

        # — Mean MAPQ panel — pre-computed per window —
        ax_mean.fill_between(x, mean_mapq_win, color=MAPQ_B, alpha=0.95, linewidth=0)
        ax_mean.axhline(contig_mean_mapq, **MEAN_STYLE,
                        label=f"Contig mean MAPQ  {contig_mean_mapq:.2f}")
        ax_mean.set_ylabel("Mean MAPQ", color=TEXT_DIM, fontsize=9, fontfamily=FONT_MAIN)
        ax_mean.set_ylim(0, ylim_top_mapq)
        ax_mean.legend(fontsize=8, facecolor=AX_BG, edgecolor=GRID,
                       labelcolor=TEXT, framealpha=0.9,
                       loc="upper left", bbox_to_anchor=(1.01, 1), bbox_transform=ax_mean.transAxes)

        # — Softclip % panel — pre-computed per window —
        sc_max = np.nanmax(sc_win) if len(sc_win) else 0.1
        ylim_top_sc = max(sc_max * 1.05, 0.1)

        ax_sc.fill_between(x, sc_win, color=SC_A, alpha=0.95, linewidth=0.5, edgecolor=SC_A)
        ax_sc.axhline(contig_softclip, **SOFTCLIP_STYLE,
        ...
        ax_sc.legend(fontsize=8, facecolor=AX_BG, edgecolor=GRID,
                     labelcolor=TEXT, framealpha=0.9,
                     loc="upper left", bbox_to_anchor=(1.01, 1), bbox_transform=ax_sc.transAxes)
        ax_sc.set_xlabel(f"Position ({unit})", color=TEXT_DIM, fontsize=9, fontfamily=FONT_MAIN)
        ax_sc.xaxis.set_major_formatter(
            ticker.FuncFormatter(lambda v, _: f"{v:,.1f}" if divisor > 1 else f"{int(v):,}")
        )
...
def fig_per_contig_rolling(df_win, df_sum, figures_folder, rolling_target_bp):
    ...
    if df.empty:
        print("[WARN] No per-contig data found in window_stats.csv — skipping rolling contig figures.")
        return
    ...
    for i, chrom in enumerate(chroms, 1):
        ...
        cov_rolling      = pd.Series(cov).rolling(rolling_n, center=True, min_periods=1).mean().values
        med_mapq_rolling  = pd.Series(med_mapq_win).rolling(rolling_n, center=True, min_periods=1).mean().values
        mean_mapq_rolling = pd.Series(mean_mapq_win).rolling(rolling_n, center=True, min_periods=1).mean().values
        sc_rolling       = pd.Series(sc_win).rolling(rolling_n, center=True, min_periods=1).mean().values
        ...
        ax_cov.plot(x, cov_rolling, color=COV_A, linewidth=1.5, zorder=4,
                    label="Rolling mean cov")
        ax_cov.axhline(mean_cov,   **MEAN_STYLE_DASH,   label=f"Contig mean cov {mean_cov:.1f}×")
        ax_cov.axhline(median_cov, **MEDIAN_STYLE_DASH, label=f"Contig median cov {median_cov:.1f}×")
        ax_cov.set_ylabel("Coverage", color=TEXT_DIM, fontsize=9, fontfamily=FONT_MAIN)
        ax_cov.set_ylim(0, cov_ylim_top)
        ax_cov.set_xlim(*xlim)
        ax_cov.legend(fontsize=8, facecolor=AX_BG, edgecolor=GRID,
        ...
        # — Merged MAPQ panel —
        ax_mapq.fill_between(x, mean_mapq_rolling, med_mapq_rolling,
                             alpha=0.35, color=GRID, linewidth=0, zorder=3)
        ax_mapq.plot(x, mean_mapq_rolling, color=MAPQ_B, linewidth=1.0, zorder=4,
                     label="Rolling mean MAPQ")
        ax_mapq.plot(x, med_mapq_rolling,  color=MAPQ_A, linewidth=1.0, zorder=4,
                     label="Rolling median MAPQ")
        ax_mapq.axhline(contig_mean_mapq, **MEAN_STYLE_DASH,
                        label=f"Contig mean MAPQ  {contig_mean_mapq:.2f}")
        ax_mapq.axhline(contig_med_mapq,  **MEDIAN_STYLE_DASH,
                        label=f"Contig median MAPQ  {contig_med_mapq:.1f}")
        mapq_data_max = np.nanmax([*mean_mapq_rolling, *med_mapq_rolling, contig_mean_mapq, contig_med_mapq])
        ylim_top_mapq = max(65, mapq_data_max * 1.1) if np.isfinite(mapq_data_max) else 65
        ax_mapq.set_ylabel("MAPQ", color=TEXT_DIM, fontsize=9, fontfamily=FONT_MAIN)
        ax_mapq.set_ylim(0, ylim_top_mapq)
        ax_mapq.legend(fontsize=8, facecolor=AX_BG, edgecolor=GRID,
                       labelcolor=TEXT, framealpha=0.9,
                       loc="upper left", bbox_to_anchor=(1.01, 1), bbox_transform=ax_mapq.transAxes)
        ...
        ax_sc.plot(x, sc_rolling, color=SC_A, linewidth=1.2, zorder=4,
                   label="Rolling mean softclip %")
        ax_sc.axhline(contig_softclip, **{**SOFTCLIP_STYLE, "linestyle": "--"},
                      label=f"Contig softclip %  {contig_softclip:.2f}")
        ax_sc.set_ylabel("Softclip %", color=TEXT_DIM, fontsize=9, fontfamily=FONT_MAIN)
